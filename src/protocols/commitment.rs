...
impl Node {
    /// SEND
    pub fn send_message(&mut self, dest: NodeId, msg: &str) -> std::io::Result<PeerReviewMsg> {
        let prev_hash = self.logger.get_current_hash();

        let sig = self.logger.log_send(dest, msg, &mut self.keypair)?;
    ...
    fn verify_send_message(&mut self, sender_id: NodeId, pr_msg: &PeerReviewMsg) -> bool {
    ...
    pub fn recv_message(
        ...
        if !self.verify_send_message(sender_id, pr_msg) {
            let _ = self.create_send_challenge(sender_id, send_msg.clone(), sender_auth.clone()); // TODO
        ...
        self.send_to_witnesses(
            sender_id,
            PeerReviewMsg::AuthenticatorBroadcast(AuthenticatorBroadcast {
                auth_node: sender_id,
        ...
        self.logger
            .log_recv(sender_id, send_msg.seq, send_msg.sig, &send_msg.msg)?;

        let recv_log = &self.logger.get_log(self.logger.s_k, self.logger.s_k)?[0];

        let ack_signature = self.logger.log_send(sender_id, "", &mut self.keypair)?;
    ...
    pub fn verify_ack(&mut self, receiver_id: NodeId, pr_msg: &PeerReviewMsg) -> bool {
...
pub fn calculate_recv_content_hash(sender_id: NodeId, sender_seq: usize, msg: &str) -> [u8; 32] {
    let mut hasher = Sha256::new();
