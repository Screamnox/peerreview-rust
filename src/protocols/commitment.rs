...
impl PeerReviewNode {
    ...
    pub fn send_message(
        ...
        println!("[Nœud {}] Message SEND créé pour le nœud {} (seq={}, prev_hash={:02x}{:02x}...)", 
    ...
    pub fn verify_send_message(
        ...
        println!("[Nœud {}] Vérification du message SEND (seq={}, prev_hash={:02x}{:02x}...)", self.node_id, seq_num, prev_hash[0], prev_hash[1]);
        ...
        println!("[Nœud {}] Calcul de H(ck) pour ck = {{ {}, {} }}", self.node_id, msg.dest, message);
        ...
        println!("[Nœud {}] Calcul de ĥk = H(hk-1 || sk || SEND || H(ck)). ĥk = {}", self.node_id, hex::encode(h_k_computed));
        ...
        println!("[Nœud {}] Vérification de la signature pour (sk={}, ĥk={})", self.node_id, seq_num, hex::encode(h_k_computed));

        let signature_obj = match ed25519_dalek::Signature::from_bytes(&signature) {
            Ok(sig) => sig,
            Err(_) => {
                println!("[Nœud {}] Erreur: Format de signature invalide", self.node_id);
                return false;
            }
        };
        ...
        if sender_public_key.verify(&signed_data, &signature_obj).is_err() {
            println!("[Nœud {}] ✗ Signature invalide: ĥk = {} ne correspond pas au hk signé par le nœud {}", 
                     self.node_id, hex::encode(h_k_computed), sender_id);
            return false;
        }
        
        // La signature est valide, donc hk == ĥk est prouvé mathématiquement
        println!("[Nœud {}] ✓ Signature valide: hk == ĥk = {}", self.node_id, hex::encode(h_k_computed));

        println!("[Nœud {}] Message SEND vérifié avec succès du nœud {} (seq={})", 
    ...
    pub fn receive_message(
        ...
        let sender_public_key = match self.peer_public_keys.get(&sender_id) {
            Some(key) => key,
            None => {
                println!("[Nœud {}] Erreur: Clé publique du nœud {} non enregistrée", 
    ...
    pub fn verify_recv_message(
        ...
        if receiver_public_key.verify(&signed_data, &signature_obj).is_err() {
            println!("[Nœud {}] Erreur: Signature invalide de l'acquittement du nœud {}", 
        ...
        println!("[Nœud {}] Acquittement vérifié avec succès du nœud {} (seq={})", 
    ...
    pub fn send_with_acknowledgment(
        ...
        if let Some(ack) = ack_received {
            let is_valid = self.verify_recv_message(&ack, receiver_id, &receiver_public_key, original_seq, message);
            if is_valid {
                return Ok(true);
            }
        }
