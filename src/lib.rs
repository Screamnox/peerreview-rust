...
impl PeerReviewRuntime {
    ...
    pub fn run(mut self) -> std::io::Result<()> {
        println!("[Runtime] PeerReview runtime started for node {}", self.node.id);
        ...
        let periodic_handle = thread::spawn(move || {
            loop {
                ...
                if let Err(e) = periodic_task_tx.send(RuntimeTask::PeriodicEvidenceCollection) {
                    eprintln!("[PeriodicThread] Failed to send PeriodicEvidenceCollection task: {}", e);
                }
            }
        ...
        loop {
            ...
            match self.task_rx.recv() {   // Note: No timeout
                Ok(task) => {
    ...
    fn handle_receive_message(&mut self, sender: NodeId, msg: PeerReviewMsg) -> std::io::Result<()> {
        match msg {
            // Commitment Protocol
            ...
            PeerReviewMsg::AuthenticatorBroadcast(ref auth_broadcast) => {
                println!("[Runtime] Received authenticator broadcast for node {}", 
                    auth_broadcast.auth_node);
                
                // Store authenticator as a witness
                let threshold_exceeded = self.node.store_authenticator(
                    auth_broadcast.auth_node,
                    auth_broadcast.auth.clone()
                );
                
                // If threshold exceeded, trigger consistency challenge
                if threshold_exceeded {
                    println!("[Runtime] Authenticator threshold exceeded for {}, triggering challenge",
                        auth_broadcast.auth_node);
                    // TODO track the send msg
                    self.node.send_consistency_challenge(auth_broadcast.auth_node)?;
            ...
            PeerReviewMsg::ChallengeRequest(ref challenge_req) => {
                println!("[Runtime] Received challenge request from {} for node {}", 
                ...
                if challenge_req.challenge.target == self.node.id {
                    ...
                    self.node.witnesses_recv_challenge(challenge_req.challenge.clone())?;
                }
            }
            ...
            PeerReviewMsg::EvidenceRequest(ref req) => {
                println!("[Runtime] Received evidence request from {} about node {}", 
                    sender, req.target);
                self.node.handle_evidence_request(sender, req)?;
            }
            
            PeerReviewMsg::EvidenceResponse(ref resp) => {
                println!("[Runtime] Received evidence response about node {}", resp.target);
            ...
            PeerReviewMsg::ProofBroadcast(ref proof_broadcast) => {
                println!("[Runtime] Received proof broadcast: {} accuses {}", 
                    proof_broadcast.proof.accuser_node,
                    proof_broadcast.proof.faulty_node);

                self.node.recv_exposure_proof(sender, &proof_broadcast.proof);
            }
        }
    ...
    fn check_timeouts(&mut self) -> std::io::Result<()> {
        ...
        let timed_out: Vec<PendingOperation> = self.pending_operations
            .iter()
        ...
        // Handle timeouts
        for op in timed_out {
            println!("[Runtime] Operation timed out for peer {}: {:?}", 
            ...
            match op.operation_type {
                OperationType::MessageSend => {
                    ...
                    self.node.create_send_challenge(op.peer_id, send_msg, sender_auth)?;   // TODO fix
                }
                ...
                OperationType::ChallengeSend => {
                    ...
                    self.node.set_peer_status(op.peer_id, types::PeerStatus::Suspected);
                }
            ...
            self.pending_operations.retain(|p| {
                !(p.peer_id == op.peer_id && 
                  std::mem::discriminant(&p.operation_type) == std::mem::discriminant(&op.operation_type))
            });
        }
